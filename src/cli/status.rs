...
pub async fn run(config: &Config, json: bool, position_filter: Option<String>) -> anyhow::Result<()> {
    ...
    let positions_to_check: Vec<_> = if let Some(ref filter) = position_filter {
        positions.into_iter().filter(|p| p.name == *filter).collect()
    } else {
        positions
    };
    ...
    if positions_to_check.is_empty() {
        if json {
            ...
            println!("Position '{}' not found", position_filter.unwrap_or_default());
        }
        return Ok(());
    }
    ...
    for position in positions_to_check {
        let status = if position.grpc_port > 0 {
            match client.connect_position(position.clone()).await {
                Ok(mut pos_client) => {
                    match pos_client.get_acquisition_info().await {
                        Ok(info) => PositionStatus {
                            name: position.name.clone(),
                            state: info.state.label().to_string(),
                            run_id: if info.run_id.is_empty() { None } else { Some(info.run_id) },
    ...
    if json {
        ...
        for status in results {
            println!("Position: {}{}", status.name, if status.simulated { " (simulated)" } else { "" });
